...
data class Resource<out T>(val status: FetchingState, val data: T?, val message: String?) {
    companion object {
        fun <T> success(data: T?): Resource<T> {
            return Resource(FetchingState.SUCCESS, data, null)
        }

        fun <T> error(msg: String, data: T?): Resource<T> {
            return Resource(FetchingState.ERROR, data, msg)
        }

        fun <T> loading(data: T?): Resource<T> {
            return Resource(FetchingState.FETCHING, data, null)
        }
    }
}
